...
def build_model(n):
    ...
    docs = list(df.iloc[:, 1].values)
    kw_tfidf = {'max_df': 0.8, 'stop_words': 'english', 'min_df':0.015, 'tokenizer':None, 'word_pattern': "[\w'\.\-]{2,}\w"}
    kw_nmf = {'n_components': 100, 'max_iter': 300}
    kw_kmeans = {'n_clusters': 20}
    model = Movier(kw_tfidf=kw_tfidf, kw_nmf=kw_nmf, kw_kmeans=kw_kmeans)
    model.fit(docs)
...
if __name__ == '__main__':
    ...
    # df = pd.read_csv('../data/txt/clean/subtext2013', delimiter='\t')
    # docs = list(df.iloc[:, 1].values)
    # kw_tfidf = {'max_df': 0.8, 'stop_words': 'english', 'min_df':10, 'tokenizer':None}
    # kw_nmf = {'n_components': 100, 'max_iter': 300}
    # kw_kmeans = {'n_clusters': 20}
    # model = Movier(kw_tfidf=kw_tfidf, kw_nmf=kw_nmf, kw_kmeans=kw_kmeans)
    # model.fit(docs)
    ...
    model = Movier(kw_tfidf=kw_tfidf, kw_nmf=kw_nmf, kw_kmeans=kw_kmeans)
    model.fit(docs)
    pickle.dump(model, open('random40_onlyw.pkl', 'wb'))
